...
class DBHelper {
  ...
  /**
   * Fetch a restaurant by its Review ID.
   */
  ...
  static getStaticAllRestaurantsMapImage(restaurants) {
    ...
    let mapURL = `http://maps.googleapis.com/maps/api/staticmap?center=${loc.lat},${loc.lng}&zoom=12&size=${document.documentElement.clientWidth}x400&markers=color:red`;
    restaurants.forEach(r => {
      mapURL += `|${r.latlng.lat},${r.latlng.lng}`;
    });
    mapURL += "&key=AIzaSyBmsqXunx1tcEXgNTgci0rRYxggh0Y5Bqg";
    return mapURL;
  }
  ...
  static handleFavoriteClick(id, newState) {
    ...
    DBHelper.updateFavorite(id, newState, (error, resultObj) => {
      ...
      favorite.style.background = resultObj.value ? `url("/ico/star-c.svg") no-repeat` : `url("ico/star-b.svg") no-repeat`;
    });
  }
